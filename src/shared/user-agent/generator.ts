...
const generators: {
  ...
  chrome: {
    ...
    windows: ({ chromeVersion }) => {
      return new RandExp(
        /Mozilla\/5\.0 \(Windows NT 10\.0; Win64; x64\) AppleWebKit\/537\.36 \(KHTML, like Gecko\) Chrome\/__CHROME_VERSION__ Safari\/537\.36/
      )
        .gen()
        .replace(/__CHROME_VERSION__/g, reduceVersion(chromeVersion))
    },
  ...
  firefox: {
    ...
    windows: ({ firefoxVersion }) => {
      return new RandExp(
        /Mozilla\/5\.0 \(Windows NT 10\.0; Win64; x64; rv:__FF_VERSION__\) Gecko\/20100101 Firefox\/__FF_VERSION__(||\/[a-zA-Z0-9]{9,16}(-\d{2}|))/
      )
        .gen()
        .replace(/__FF_VERSION__/g, firefoxVersion)
    },
